...
데이터가 많아짐에 효율성을 위해 따라 정리할 수 있는 공간 (수납상자, 냉장고...)

연관된 데이터를 정리해놓는 공간을 뜻한다.

### 함수

메서드라고 불리는 경우도 있다.(객체)

-> 함수는 입력과 출력으로 이루어져 있으며, 파라미터가 입력 부분이고 출력은 return이다.

### 표현식

1+1 = 2에서 1+1은 값2에 대한 표현식이다.
...
캐시 때문에 공통함수는 묶는게 서버 입장에서 비용을 절감할 수 있다.

### 라이브러리와 프레임워크

다른 사람의 코드를 이용하여 도움받는 점에서 둘은 비슷하다.

라이브러리 : 내가 만들고자하는 프로그램의 부품을 가져오는 느낌(도서관)

프레임워크 : 만들고자 하는 것이 있을 때 그게 무엇이냐에 따라서 공통적인 부분을 프레임워크가 만들고 우리는 기능에 따라 달라지는 부분만 수정할 수 있다.
(처음부터 끝까지 만들 필요가 없는 반제품)
